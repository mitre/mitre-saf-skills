...
|-------|-----------|---------|----------|
| 1 | `bd remember --global` — cross-project learnings | Yes (global DB) | Account rotations, machine changes, ALL projects |
| 2 | `bd remember` — project decision summaries | Yes (Dolt sync) | Account rotations, machine changes |
| 3 | Beads cards with notes | Yes (Dolt sync) | Account rotations, machine changes |
| 4 | `.beads/recovery-context.md` | No (local) | Compaction, new sessions |
| 5 | `.beads/recovery-prompt.md` | No (local) | Compaction, new sessions |
| 6 | Agent memory (e.g., MEMORY.md) | No (per-account) | Sessions on same account |
| 7 | `.beads/archive/` | No (local) | Historical record |
...
```bash
bd memories "<keyword>"  # Check if similar memory exists first

# Project-specific decisions (stored in project's beads DB, synced via Dolt)
bd remember "<concise decision + rationale>" --key "<descriptive-key>"

# Cross-project learnings (stored in beads_global DB, available in ALL projects)
bd remember --global "<insight that applies everywhere>" --key "<descriptive-key>"
```

**When to use `--global`:** Learnings that apply across ALL projects, not just this one. Examples: LLM failure mode mitigations, general workflow patterns, tool behavior quirks. If in doubt, use project-local — it's easy to promote later.
