...
## When This Is a Fresh Session After a Spiral

If the previous session entered a frustration-error feedback loop (corrections → sycophancy → more errors — see `project-tdd/references/llm-failure-modes.md`), this fresh session is the strongest mitigation available. Context rot is continuous with no plateau — compacting helps but doesn't reset attention weights. A new conversation does.

**What a fresh session gives you:**
- System prompt rules at position 0 with maximum attention weight
- No accumulated distractor content from prior turns
- No residual sycophancy patterns from prior correction cycles
- Recovery files provide external state without polluting the attention window

**What to watch for in recovery context:**
- If the recovery file mentions a "spiral," "corrections," or "behavioral failure" — read that section carefully
- Check if the previous session's mistakes have a pattern (rushing, scope misrepresentation, shortcut usage)
- Re-read the Deceleration Protocol in `project-tdd` before starting any card

**The rule:** A fresh session is a reset, not a fresh start. The recovery files carry forward WHAT to do. The fresh context ensures you can actually attend to HOW to do it correctly.
...
# Project-specific memories (decisions for this codebase)
bd memories

# Global memories (cross-project learnings — LLM failure modes, workflow patterns)
bd memories --global
```

Read all memories carefully — both project and global. Project memories contain codebase-specific decisions (e.g., "Use Drizzle not Prisma"). Global memories contain cross-project learnings (e.g., frustration-error feedback loop mitigations, narration test, context rot research). Do NOT contradict stored memories without discussing with the user first.
